    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="radargram" className="space-y-6">
          {/* GPR Controls */}
          <Card className="backdrop-blur bg-white/60 border-2 border-yellow-200">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
            ...
            <CardContent>
              ...
              {compareMode === 'side-by-side' ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    <div className="mb-2 text-sm text-gray-700">Raw Radargram</div>
                    <div className="relative bg-black rounded-lg overflow-hidden" style={{ height: '400px' }}>
                      {/* Raw signal placeholder */}
                      <div className="absolute inset-0 opacity-80">
                        {[...Array(40)].map((_, i) => (
                          <div key={i} className="absolute bg-white/20" style={{ left: `${i * 2}%`, top: `${(Math.sin(i * 0.4) * 15) + 40}%`, width: '2%', height: `${20 + Math.sin(i * 0.25) * 10}%` }} />
                        ))}
                      </div>
                    </div>
                  </div>
                  <div>
                    <div className="mb-2 text-sm text-gray-700">Processed Radargram with Interpretation</div>
